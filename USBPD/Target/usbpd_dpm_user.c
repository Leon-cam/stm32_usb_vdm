...
/* USER CODE BEGIN USBPD_DPM_UserCableDetection */
  ...
  case USBPD_CAD_EVENT_ATTEMC:
    {
    dbg_printf("[CAD] Port%d: Cable ATTACHED\r\n", PortNum);
    if (DPM_Params[PortNum].PE_PowerRole == USBPD_PORTPOWERROLE_SRC)
    {
      if (USBPD_OK != USBPD_PWR_IF_VBUSEnable(PortNum))
      {
        dbg_printf("[CAD] Port%d: VBUS Enable FAILED\r\n", PortNum);
        NVIC_SystemReset();
      }
      dbg_printf("[CAD] Port%d: VBUS Enabled (SRC)\r\n", PortNum);
    }
    break;
    }
  ...
  default :
    dbg_printf("[CAD] Port%d: Cable DETACHED\r\n", PortNum);
...
/* USER CODE BEGIN USBPD_DPM_Notification */
  switch(EventVal)
  {
    case USBPD_NOTIFY_POWER_EXPLICIT_CONTRACT :
      dbg_printf("[DPM] Port%d: Explicit Contract -> VDM Discovery\r\n", PortNum);
      USBPD_VDM_StartDiscovery(PortNum);
      break;
    case USBPD_NOTIFY_DETACH:
      dbg_printf("[DPM] Port%d: Detached\r\n", PortNum);
      USBPD_VDM_UserReset(PortNum);
      break;
    case USBPD_NOTIFY_HARDRESET_RX:
      dbg_printf("[DPM] Port%d: Hard Reset RX\r\n", PortNum);
      USBPD_VDM_UserReset(PortNum);
      break;
    case USBPD_NOTIFY_HARDRESET_TX:
      dbg_printf("[DPM] Port%d: Hard Reset TX\r\n", PortNum);
      USBPD_VDM_UserReset(PortNum);
      break;
    default:
      /* dbg_printf("[DPM] Port%d: Notification %d\r\n", PortNum, (int)EventVal); */
      break;
  }
